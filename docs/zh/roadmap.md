...
**应用前端** 在 v0.2 已经做出来了：有设计的窗口，而不只是终端选择器。
...
| 对话 | **已落地。** 每个 agent 一条对话，用各自的会话记录渲染，同时保留终端作为兜底。适配器都照真实会话记录写（Claude 的、511 个 Codex rollout、73 个 Grok 会话）——只有 Pi 是照 CLI 自己的格式文档写的，量不到的形状标了 `unverified`。 |
